...
# just use for now while there hasn't yet been a top-3 quickfire or a LCK
tc <- tc %>%
  mutate(
    qf_top3 = 0,
    lck_win = 0,
...
server <- function(input, output, session){
  ...
  output$teamtitle <- renderUI({
    ...
    if(length(s)){
      ...
      standings <- tc %>%
        mutate(team_url = paste("<img src = ' ",team_url,"' height = '55'></img>")) %>%
        group_by(team_url, team) %>%
        summarize(
          chefs_remaining = 5 - sum(eliminated),
          lck_chefs = sum(status == "LCK"),
  ...
  output$chefstats <- DT::renderDataTable({
    ...
    chefstats <- tc %>%
      mutate(chef_url = paste("<img src = ' ",chef_url,"' height = '45'></img>")) %>%
      mutate(team_url = paste("<img src = ' ",team_url,"' height = '45'></img>")) %>%
      group_by(chef_url, cheftestant, team_url) %>%
      summarize(
        points = sum(weekly_points),
        elim_wins = sum(elim_win),
        elim_top3s = sum(elim_top3),
      ...
      left_join(chefs, by = "cheftestant") %>%
      arrange(status) %>%
      select(chef_url, cheftestant, team_url, status, everything())
    ...
    table.on( 'order.dt search.dt', function () {
      ...
      options = list(
        ...
        columnDefs = list(
          ...
          list(orderable = FALSE, targets = c(1,2,3,4))
        ),
        pageLength = 15
      )
    ) %>%
